...
class TestReferenceEvaluator:
    ...
    def test_reference_evaluator_no_attribute(self):
        ...
        assert sess.input_names == ["B01", "B11", "B21"]
        assert sess.output_names == ["D0"]
        assert sess.opsets == {"": 10, "com.microsoft": 1}
    ...
    def test_reference_evaluator_no_attribute_intermediate(self):
        ...
        expected = (x + y) * (y - z)
        assert_allclose(res["D0"], expected)
    ...
    def test_one_hot_string_values(self):
        node = make_node("OneHot", ["indices", "depth", "values"], ["output"], axis=-1)
        indices = np.array([0, 1], dtype=np.int64)
        depth = np.array(2, dtype=np.int64)
        values = np.array(["off", "on"])
        expected = np.array([["on", "off"], ["off", "on"]])
    ...
    def test_global_max_pool_spatial_ranks(self, shape, dtype, layout):
        ...
        got = self._run_global_max_pool(x)

        assert got.shape == (shape[0], shape[1], *(1,) * (len(shape) - 2))
        assert got.dtype == dtype
        assert_allclose(got, expected)

    def test_global_max_pool_nan(self):
    ...
    def test_global_max_pool_zero_batch(self, shape):
    ...
    def test_global_max_pool_empty_spatial_dimension(self, shape):
        x = np.empty(shape, dtype=np.float32)
